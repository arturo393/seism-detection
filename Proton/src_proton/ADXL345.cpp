...
writeRegister8(ADXL345_REG_BW_RATE, 0x0A);
writeRegister8(ADXL345_REG_DATA_FORMAT, 0x0B);
writeRegister8(ADXL345_REG_INT_ENABLE, 0x80);
...
// Low Pass Filter
acc ADXL345::lowPassFilter(acc acc, float alpha)
{
    f.XAxis = acc.XAxis * alpha + (f.XAxis * (1.0 - alpha));
    f.YAxis = acc.YAxis * alpha + (f.YAxis * (1.0 - alpha));
    f.ZAxis = acc.ZAxis * alpha + (f.ZAxis * (1.0 - alpha));
    return f;
}

// Read raw values
acc ADXL345::readRaw(void)
{
...
acc ADXL345::readNormalize(float gravityFactor)
{
    readRaw();
...
void ADXL345::useInterrupt(adxl345_int_t interrupt)
{
    ...
    //writeRegister8(ADXL345_REG_DATA_FORMAT,0x20);
    writeRegister8(ADXL345_REG_INT_ENABLE, 0x80);
