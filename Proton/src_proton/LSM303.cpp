...
void LSM303::enableDefault(void)
{
  ...
  if (_device == device_D)
  {
    ...
    writeReg(CTRL1, 0x57);
   // ACC DATA_READY INT1 interrupt enable
  //  writeReg(CTRL3, 0x04);
  ...
  else
  {
    ...
    if (_device == device_DLHC)
    {
      ...
      // 0x47 = 0b01100111
      // ODR = 0110 (200 Hz ODR); LPen = 0 (normal mode); Zen = Yen = Xen = 1 (all axes enabled)
      writeAccReg(CTRL_REG1_A, 0x67);

      // 0x47 = 0b10101000
      // HPM = 00 (normal mode); HPCF = 10 (?); FDS = 0 (normal mode);  HPCLICK = 0 (filter bypassed); HPIS2 = 0 (filter bypassed); HPIS1 = 0 (filter bypassed)
      //writeAccReg(CTRL_REG2_A, 0xA8);

    }
    else // DLM, DLH
    {
      Serial.println("DLM/DLH");
      // 0x00 = 0b00000000
      ...
      // 0x03 = 0b00000011
      // HPCF = 11 (1 hz)
     writeAccReg(CTRL_REG2_A, 0x03);

      // 0x37 = 0b00110111
      // PM = 001 (normal mode); DR = 10 (50 Hz ODR); Zen = Yen = Xen = 1 (all axes enabled)
      writeAccReg(CTRL_REG1_A, 0x37);

/*
      // 0x04 = 00000100
      // LIR2 = 1
      // LATCH INT on INT1_SRC_A
      writeAccReg(CTRL_REG3_A, 0x06);
*/
    }
...
Interrupt LSM303::isInterrupt(){

  uint8_t data = readReg(STATUS_REG_A);
  i.isO = ((data >> LSM303DLM_0) & 1);
  i.isInterruptActive= ((data >> LSM303DLM_IA) & 1);
  i.isZhigh = ((data >> LSM303DLM_XH) & 1);
  i.isZlow = ((data >> LSM303DLM_ZL) & 1);
  i.isZYXDA = ((data >> LSM303DLM_YH) & 1);
  i.isYlow = ((data >> LSM303DLM_YL) & 1);
  i.isXhigh = ((data >> LSM303DLM_XH) & 1);
  i.isXlow = ((data >> LSM303DLM_XL) & 1);

return i;
}
bool LSM303::isDataReady(){
  uint8_t data = readReg(SR_REG_M);
return ((data >> DRDY) & 1);
}
