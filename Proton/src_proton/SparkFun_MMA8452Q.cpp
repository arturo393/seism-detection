...
Serial.println("STANDBY");
	...
	//Serial.println("Hifh pass filter enable, 2g range");
	// XYZ_DATA_CFG = 0x00010000
	// HPF_OUT = high pass filter enable , 2g range
	//writeRegister(XYZ_DATA_CFG, 0x10);
	//Serial.println(" 1 Hz high pass filter");
	// HP_FILTER_CUTOFF = 0x00000010
	// 1 Hz high pass filter
	//writeRegister(HP_FILTER_CUTOFF, 0x04);
	// CTRL_REG2 = 0x00000010
	Serial.println("Hihg resolution");
	writeRegister(CTRL_REG2, 0x02);
	// CTRL_REG4 = 0x00000001
	Serial.println("INT_EN_DRDY = interrupt data ready enable");
		writeRegister(CTRL_REG4, 0x01);
	// CTRL_REG5 = 0x00000001
	Serial.println("INT1 routing");
	writeRegister(CTRL_REG5, 0x01);
	// CTRL_REG1 = 0x00011111
	Serial.println("ODR 100 Hz , LNOISE  , ACTIVE");
	writeRegister(CTRL_REG1, 0x1D);
...
byte MMA8452Q::init(MMA8452Q_Scale fsr, MMA8452Q_ODR odr,MMA8452Q_HP_BW bw)
{
...
void MMA8452Q::setHPFilter(MMA8452Q_HP_BW bw){

	byte cfg = readRegister(XYZ_DATA_CFG);
	cfg &= 0xEF; // Mask out scale bits
	cfg |= (1 >> 4);  // Neat trick, see page 22. 00 = 2G, 01 = 4A, 10 = 8G
	writeRegister(XYZ_DATA_CFG, cfg);
	// Must be in standby mode to make changes!!!
	byte ctrl = readRegister(HP_FILTER_CUTOFF);
	ctrl &= 0xFC; // Mask out data rate bits
	ctrl |= (bw << 2);
...
byte MMA8452Q::readRegister(MMA8452Q_Register reg)
{
  ...
  timeover=millis();
	//while(!Wire.available()); //Wait for the data to come back or 2 seconds
