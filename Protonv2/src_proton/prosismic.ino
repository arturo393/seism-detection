...
int SendToServer(){
  ...
  ZC, IQR, CAV, latitude, longitude);
   Particle.publish("Sismo", lines, PRIVATE);
}
...
void locationCallback(float lat, float lon, float accuracy) {
  ...
  dtostrf(lat, 7 , 5, latitude); // Leave room for too large numbers!
  dtostrf(lon, 7 , 5, longitude); // Leave room for too large numbers!
