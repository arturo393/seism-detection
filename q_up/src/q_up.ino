...
const int RestartTime = 2 * SEGUNDOS_POR_MINUTO;  // tiempo en recalcular el
const int OnlineTime = 2*SEGUNDOS_POR_HORA;                         // offset, las referencias y
...
int SAMPLES = 200;  // numero de muestras para el calculo
short c_muestra;    // cuentas de la ventana de analisis
...
short RSL, RSLref, RSLmin, RSLmax;   // Parametro Mr. P (Razon entre promedios)
long IQR, IQRref, IQRmin, IQRmax;    // Rango intercuartílico
short ZC, ZCref, ZCmax, ZCmin;       // tasa de cruces por cero
...
short trigger;  // 0:notrigger / 1:RSL / 2:ZIC / 3:IC
...
short tc_mcheck;      // muestras que está en estado de movimiento
short tc_dpcheck;     // cuantas que está en estado de desplazamiento
short tc_temp1;       // counter temporal for DP event
short tc_temp2;       // counter temporal for DP event
short tc_cserver;     // muestras que el servidor no responde
short tc_dpchecktmp;  // temporal para estado de desplazamiento
short tc_referencia;  // muestras para calcular las referencias
short tc_geo;         // cuentas en que no se encuentra la georefencia de google maps

// contadores de estado
short ce_mcheck;   //  veces que está en movimiento
short ce_dpcheck;  //  veces que está en desplazamiento
short ce_event;    //  cuentas en estado de detección
...
int t_locationsync = 2*SEGUNDOS_POR_MINUTO;

// variables de acelerómetro LSM303
//Adafruit_LSM303_Accel_Unified accel = Adafruit_LSM303_Accel_Unified(54321);
//sensors_event_t event;  // para lectura de la aceleración
//sensor_t sensor;        // para lectura de información del dispositivo
String ID;
...
void setup() {
 ...
 // Particle.publish("particle/device/name");
  ...
  // Tiempo (se sincroniza automaticamente con el Cloud)
 // blinking_delay(1000);
...
void loop() {
  ...
  if (millis() - t_muestreo >= INTERVALO) {
    ...
    snprintf(line, sizeof(line), "%dsam %d[ms] %d %d %d %d", c_muestra,
    ...
    snprintf(line, sizeof(line), " %d[ms] %d %d %d %d", t_dparam, acc_xMax,
 ...
 //   Particle.publish("particle/device/name");
  ...
  if ((Time.now() - tc_reset) >= (RestartTime)) {
    ...
    /* Status send*/
  if ((Time.now() - t_online) >= (SEGUNDOS_POR_HORA * OnlineTime)) {
    t_online = Time.now();
    sendStatus(1);
  }
}
...
snprintf(line, sizeof(line), "%s;%lu;%d",_id, Time.now(),_status);
...
int CalcRef(int _i, bool _iscalc, int _samples) {
  ...
  if (_i < _samples) {  // use total _samples for calculation
    ...
    snprintf(line, sizeof(line), " %d %d %d %d %d %d %d %d %d %d %d %d", ZCmax,
...
int sendPost(time_t _time) {
  ...
  ID.toCharArray(_id,10);
  snprintf(line, sizeof(line),
           "%s;%lu;%lu;%d;%lu;%d;%d;%d;%d;%d;%d;%s;%s",_id, _time,
           (millis() - t_millis) % 1000, ACNmax, trigger, ZCref, IQRref, CAVref,
...
int postRequest(time_t _time) {
  ...
  snprintf(line, sizeof(line), "%s;%lu;%lu;%d;%lu;%d;%d;%d;%d;%d;%d;%s;%s",
           _id, _time, (millis() - t_millis) % 1000, ACNmax, trigger,
...
time_t calcParam(int _sample) {
  ...
  if (X.size() >= SAMPLES) {
    ...
    for (int jj = 0; jj < X.size(); jj++) {
      ...
      /*
snprintf(line, sizeof(line), "% 3d % 2d % 2d % 2d % 4d ", sample, X[jj],Y[jj],
Z[jj], XYZ[jj] );
Serial.println(line);
*/
    }
...
void displaySensorDetails(void) {
  ...
  Serial.println("");
  delay(500);
  ID = String(sensor.name + System.deviceID());
}
*/
